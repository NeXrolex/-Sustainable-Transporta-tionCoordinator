...
MAX_HISTORY      = 60          # data points kept for graphs
...
# ─── Arrival probability per hour block ───────────────────
# (hour_start, hour_end, arrive_prob, depart_prob)
# Realistic profile: 70-85% occupancy most of the day.
# Brief full-lot at 12:00-12:15, then quick recovery.
# (hour_start, hour_end, arrive_prob, depart_prob)
HOURLY_PROFILE = [
    (0,  7,  0.00, 0.00),   # Night – closed
    (7,  8,  0.70, 0.04),   # Morning rush – fills to ~70%
    (8,  10, 0.45, 0.08),   # Steady arrivals, moderate turnover → 75-80%
    (10, 12, 0.35, 0.10),   # Slight slowdown, still ~75%
    (12, 13, 0.75, 0.08),   # Lunch spike → brief peak, may hit 100%
    (13, 14, 0.20, 0.30),   # Quick recovery → back to ~75% by 12:15 sim
    (14, 17, 0.25, 0.12),   # Stable afternoon ~70%
    (17, 18, 0.10, 0.40),   # Departures accelerate
    (18, 20, 0.02, 0.75),   # Mass exit
    (20, 24, 0.00, 0.40),   # Night wind-down
]
...
class ParkingSimEngine:
    ...
    def __init__(self, callback_tick):
        self.callback_tick   = callback_tick   # called every simulated minute
        self.env             = simpy.Environment()
        self._running        = False
        self._after_id       = None            # tkinter after() id
        ...
        # Stats
        self.sim_hour        = 7       # simulation starts at 07:00
        self.sim_minute      = 0
        self.raining         = False
        self.rejected        = 0
        ...
        # History for graphs (deques for O(1) append/pop)
        self.hist_hour       = deque(maxlen=MAX_HISTORY)
        self.hist_occupied   = deque(maxlen=MAX_HISTORY)
        self.hist_available  = deque(maxlen=MAX_HISTORY)
        self.hist_reserved   = deque(maxlen=MAX_HISTORY)
        self.hist_priority   = deque(maxlen=MAX_HISTORY)
        self.hist_blocked    = deque(maxlen=MAX_HISTORY)
        self.hist_rejected   = deque(maxlen=MAX_HISTORY)
    ...
    def _tick_process(self, env):
        while True:
            ...
            if self.sim_minute >= 60:
                ...
                # random rain toggle per hour
                self.raining = random.random() < RAIN_PROBABILITY

            # ── arrivals ──
            # At most 2 vehicles per tick; 1 is the norm.
            # Reservation chance is low (10%) – most park directly.
            ap = self._arrival_prob()
            h  = self.sim_hour
            max_arrivals = 2 if (7 <= h < 8 or 12 <= h < 13) else 1
            ...
            for _ in range(num_arrivals):
                ...
                if is_carpool:
                    ...
                    if target:
                        # Only 10% of vehicles reserve – most park directly
                        if random.random() < 0.10:
                            target.transition(RESERVED)
                            # reservation converts to occupied quickly
            ...
            # ── snapshot history ──
            occ  = self._count(OCCUPIED)
            ava  = self._count(AVAILABLE)
            res  = self._count(RESERVED)
            pri  = self._count(PRIORITY)
            blk  = self._count(BLOCKED)
            t    = self.sim_hour + self.sim_minute / 60
            self.hist_hour.append(t)
            self.hist_occupied.append(occ)
            self.hist_available.append(ava)
            self.hist_reserved.append(res)
            self.hist_priority.append(pri)
            self.hist_blocked.append(blk)
            self.hist_rejected.append(self.rejected)
...
class ParkingAutomatonApp:
    ...
    def _build_ui(self):
        ...
        grid_card.pack(fill="x", pady=(0, 8))
        ...
        self._grid_canvas.pack(fill="x", padx=10, pady=(0, 10))
        ...
        # charts card
        charts_card = tk.Frame(center, bg=BG_CARD)
        charts_card.pack(fill="both", expand=True)

        tk.Label(
            charts_card, text="REAL-TIME METRICS",
            bg=BG_CARD, fg=FG_WHITE,
            font=("Segoe UI", 10, "bold")
        ).pack(anchor="w", padx=12, pady=(10, 4))

        self._chart_frame = tk.Frame(charts_card, bg=BG_CARD)
    ...
    def _build_charts(self):
        ...
        self._fig = plt.Figure(figsize=(9, 3.0), dpi=96)
        self._fig.patch.set_facecolor(BG_CARD)

        gs = self._fig.add_gridspec(1, 3, wspace=0.38, left=0.06, right=0.97,
        ...
        def style_ax(ax, title):
            ...
            ax.set_title(title, color=FG_WHITE, fontsize=8, pad=6,
                         fontname="Segoe UI")
        ...
        self._ax3 = self._fig.add_subplot(gs[2])

        style_ax(self._ax1, "Occupation vs Hour")
        style_ax(self._ax2, "Space States")
        style_ax(self._ax3, "Rejected Vehicles")

        self._ax1.set_xlabel("Hour", color=FG_MUTED, fontsize=7)
        self._ax1.set_ylabel("Spaces", color=FG_MUTED, fontsize=7)
        self._ax2.set_xlabel("Hour", color=FG_MUTED, fontsize=7)
        self._ax3.set_xlabel("Hour", color=FG_MUTED, fontsize=7)

        self._canvas_chart = FigureCanvasTkAgg(self._fig, master=self._chart_frame)
        self._canvas_chart.get_tk_widget().pack(fill="both", expand=True)
        self._canvas_chart.draw()

    def _refresh_charts(self):
        e = self.engine
        if len(e.hist_hour) < 2:
            return

        x   = list(e.hist_hour)
        occ = list(e.hist_occupied)
        ava = list(e.hist_available)
        res = list(e.hist_reserved)
        pri = list(e.hist_priority)
        blk = list(e.hist_blocked)
        rej = list(e.hist_rejected)

        # Chart 1: occupied line
        self._ax1.cla()
        self._ax1.set_facecolor(BG_PANEL)
        self._ax1.set_title("Occupation vs Hour", color=FG_WHITE,
                             fontsize=8, pad=6, fontname="Segoe UI")
        self._ax1.fill_between(x, occ, alpha=0.25,
                               color=STATE_COLOR[OCCUPIED])
        self._ax1.plot(x, occ, color=STATE_COLOR[OCCUPIED],
                       linewidth=1.5, label="Occupied")
        self._ax1.axhline(y=TOTAL_SPACES * OVERFLOW_ALERT,
                          color="#e74c3c", linestyle="--",
                          linewidth=0.8, alpha=0.7)
        self._ax1.set_xlabel("Hour", color=FG_MUTED, fontsize=7)
        self._ax1.set_ylabel("Spaces", color=FG_MUTED, fontsize=7)
        self._ax1.tick_params(colors=FG_MUTED, labelsize=7)
        for sp in self._ax1.spines.values():
            sp.set_edgecolor("#1e3a4a")
        self._ax1.yaxis.grid(True, color="#1e3a4a", linewidth=0.5)
        self._ax1.set_axisbelow(True)
        self._ax1.set_ylim(0, TOTAL_SPACES + 5)

        # Chart 2: stacked states
        self._ax2.cla()
        self._ax2.set_facecolor(BG_PANEL)
        self._ax2.set_title("Space States", color=FG_WHITE,
                             fontsize=8, pad=6, fontname="Segoe UI")
        self._ax2.stackplot(
            x,
            ava, res, occ, blk, pri,
            colors=[
                STATE_COLOR[AVAILABLE],
                STATE_COLOR[RESERVED],
                STATE_COLOR[OCCUPIED],
                STATE_COLOR[BLOCKED],
                STATE_COLOR[PRIORITY],
            ],
            alpha=0.85,
            labels=["Avail", "Reserved", "Occupied", "Blocked", "Priority"]
        )
        self._ax2.set_xlabel("Hour", color=FG_MUTED, fontsize=7)
        self._ax2.tick_params(colors=FG_MUTED, labelsize=7)
        for sp in self._ax2.spines.values():
            sp.set_edgecolor("#1e3a4a")
        self._ax2.yaxis.grid(True, color="#1e3a4a", linewidth=0.5)
        self._ax2.set_axisbelow(True)
        self._ax2.set_ylim(0, TOTAL_SPACES + 5)
        leg = self._ax2.legend(
            fontsize=6, loc="upper right",
            facecolor=BG_DARK, edgecolor="#1e3a4a",
            labelcolor=FG_MUTED, ncol=2
        )

        # Chart 3: rejected
        self._ax3.cla()
        self._ax3.set_facecolor(BG_PANEL)
        self._ax3.set_title("Rejected Vehicles", color=FG_WHITE,
                             fontsize=8, pad=6, fontname="Segoe UI")
        self._ax3.fill_between(x, rej, alpha=0.30, color="#e74c3c")
        self._ax3.plot(x, rej, color="#e74c3c", linewidth=1.5)
        self._ax3.set_xlabel("Hour", color=FG_MUTED, fontsize=7)
        self._ax3.set_ylabel("Count", color=FG_MUTED, fontsize=7)
        self._ax3.tick_params(colors=FG_MUTED, labelsize=7)
        for sp in self._ax3.spines.values():
            sp.set_edgecolor("#1e3a4a")
        self._ax3.yaxis.grid(True, color="#1e3a4a", linewidth=0.5)
        self._ax3.set_axisbelow(True)
    ...
    def _refresh_metrics(self):
        ...
        if e.raining:
            self._weather_label.config(
                text="🌧  Lluvia  (+25% demand)", fg="#74b9ff"
            )
        else:
            self._weather_label.config(
